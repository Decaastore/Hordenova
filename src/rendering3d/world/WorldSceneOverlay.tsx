...
/**
 * MUNDO 3D — FASE 1/2: a background terrain/atmosphere layer, purely
 ...
 *
 * ROLLBACK NOTE — the FASE 3 tower/castle 3D integration test
 * (`TestTowerLayer`/`TestCastleLayer` from `WorldGameplayTestLayer.tsx`,
 * once mounted here) has been removed per explicit user direction: the
 * real GPU render of that baseline 3D Ironwood/castle looked worse than
 * the approved 2D art it was silently replacing (via `hidden3DTowerIds`/
 * `skipCastleBody`), so both towers and the castle now always draw their
 * normal 2D sprites again, on top of this terrain layer, exactly as
 * before FASE 3. FASE 1/2 (this terrain/atmosphere background) is
