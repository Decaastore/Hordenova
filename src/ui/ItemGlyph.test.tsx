 ...
 *
 * All 3 amulets and the MYTHIC item have real supplied artwork registered
 * by default (see itemAssets.ts); ancient_core is still honestly empty, so
 * the manually-injected-registry scenarios below use it to stay
 * independent of whether any given item currently has real art.
 */
describe("ui/ItemGlyph — real-art vs fallback-icon contract", () => {
  ...
  it("renders crown_of_the_hollow_king's real supplied artwork via <img>, not the fallback icon", () => {
  ...
  it("renders real artwork via <img> once one is registered for any item's visualAssetId", () => {
    ITEM_VISUAL_ASSETS.ancient_core!.imageSrc = "/items/runes/ancient_core.png";
    const { container } = render("ancient_core");
    const img = container.querySelector("img");
    expect(img).not.toBeNull();
    expect(img!.getAttribute("src")).toBe("/items/runes/ancient_core.png");
    expect(container.querySelector("svg")).toBeNull();
  });

  it("falls back to the hand-drawn icon if the registered image fails to load (404), never leaving a broken image", () => {
    ITEM_VISUAL_ASSETS.ancient_core!.imageSrc = "/items/runes/does-not-exist.png";
    const { container } = render("ancient_core");
    const img = container.querySelector("img")!;
