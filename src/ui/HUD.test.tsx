 ...
 * Gems had just been granted. These tests prove the currencies are now
 * impossible to conflate at a glance: separate labels, separate values
 * for Free Gems, Purchased Gems, and Gem Shards, and neither Gems value
 * ever carries a "+" suffix that could be misread as itself having just
 * increased by that amount.
 */
describe("HUD — Free Gems, Purchased Gems and Gem Shards are visually and textually unambiguous (P2 UX fix)", () => {
  it("renders Free Gems, Purchased Gems and Gem Shards as separate, independently labeled stats", () => {
    const html = renderHud(makeHud({ freeGems: 125, purchasedGems: 80, gemShards: 5 }));

    expect(html).toContain("FREE");
    expect(html).toContain("PURCHASED");
    expect(html).toContain("SHARDS"); // en.ts hud.gemShards label
    expect(html).toContain("125"); // the real Free Gems value, on its own
    expect(html).toContain("80"); // the real Purchased Gems value, on its own
  ...
  it("never renders the old ambiguous combined pattern '<gems> (+<shards>)' under one label", () => {
    ...
    expect(html).not.toContain("80 (+5)");
  });

  it("neither Gems value carries a '+' prefix (only the Shards stat does) — Gems is never shown as if it just changed by the Shards amount", () => {
    const html = renderHud(makeHud({ freeGems: 125, purchasedGems: 80, gemShards: 5 }));
    // Isolate each Stat block's rendered value text via its distinct label
    // ("FREE" / "PURCHASED" / "SHARDS") rather than raw substring search,
    // since "125" alone could coincidentally appear inside a "+125" for shards.
    const freeValueMatch = html.match(/FREE<\/div><div[^>]*>([^<]+)</);
    expect(freeValueMatch?.[1]).toBe("125");
    const purchasedValueMatch = html.match(/PURCHASED<\/div><div[^>]*>([^<]+)</);
    expect(purchasedValueMatch?.[1]).toBe("80");
  });

  it("the Gem Shards stat doesn't render at all when the balance is 0 — nothing ambiguous to show", () => {
    const html = renderHud(makeHud({ freeGems: 125, purchasedGems: 80, gemShards: 0 }));
  ...
  it("a real tooltip on the Shards stat states the actual conversion rate (10 Shards = 1 Gem) — no invented number", () => {
    const html = renderHud(makeHud({ freeGems: 0, purchasedGems: 0, gemShards: 30 }));
